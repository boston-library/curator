...
class CreateCuratorInstitutions < ActiveRecord::Migration[5.2]
  def change
    create_table :curator_institutions do |t|
      t.belongs_to :location, index: { using: :btree, name: 'index_inst_on_geo_location_nom' }, foreign_key: { to_table: :curator_controlled_terms_nomenclatures, on_delete: :nullify }
