...
class CreateCuratorControlledTermsNomenclatures < ActiveRecord::Migration[5.2]
  def change
    create_table :curator_controlled_terms_nomenclatures do |t|
      ...
      t.index "((term_data ->> 'id_from_auth')::varchar)", name: 'index_ctl_terms_nom_id_from_auth_jsonb_field'
      t.index "((term_data ->> 'basic')::boolean)", name: 'index_ctl_terms_basic_genre_jsonb_field'
    end
  end
end
