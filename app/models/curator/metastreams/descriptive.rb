...
module Curator
  class Metastreams::Descriptive < ApplicationRecord
    ...
    with_options through: :desc_terms, source: :mappable do
      has_many :genres, -> { merge(with_authority) }, source_type: Curator.controlled_terms.genre_class.to_s
      has_many :resource_types, -> { merge(with_authority) }, source_type: Curator.controlled_terms.resource_type_class.to_s
      has_many :licenses, source_type: Curator.controlled_terms.license_class.to_s
      has_many :languages, -> { merge(with_authority) }, source_type: Curator.controlled_terms.language_class.to_s
      has_many :subject_topics, -> { merge(with_authority) }, source_type: Curator.controlled_terms.subject_class.to_s
      has_many :subject_names, -> { merge(with_authority) }, source_type: Curator.controlled_terms.name_class.to_s
      has_many :subject_geos, -> { merge(with_authority) }, source_type: Curator.controlled_terms.geographic_class.to_s
    end
  end
end
