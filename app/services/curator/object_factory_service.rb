...
module Curator
  class ObjectFactoryService < ServiceClass
    ...
    def call
      ...
      begin
        Curator.digital_object_class.transaction do
          ...
          build_descriptive do |descriptive|
            ...
            descriptive.physical_location_id = physical_location(desc_json_attrs)
            ...
            desc_json_attrs.fetch(:name_role, []).each do |name_role_attrs|
              ...
              descriptive.name_roles << Curator.mappings.desc_name_role_class.new(name_role_attrs)
            end
          end
    ...
    protected
    def build_descriptive(&block)
      descriptive = Metastreams::Descriptive.new(descriptable: @digital_object)
      yield descriptive
      descriptive.save!
    end
    ...
    def build_workflow(&block)
      workflow = Curator.metastreams.workflow_class.new(workflowable: @digital_object )
      yield(workflow)
      workflow.save!
    end

    def build_administrative(&block)
      administrative = Curator.metastreams.administrative_class.new(administratable: @digital_object )
    ...
    def identifier(json_attrs={})
     ...
     def title(json_attrs={})
       ...
       Descriptives::TitleSet.new(title_primary: primary, title_other: other)
     end
