...
module Curator
  ...
  module SolrUtil
    ...
    #     end
    #
    # It is searching for any Solr object with a `Curator.config.indexable_settings.model_name_solr_field`
    # field. Then, it takes the ID and makes sure it exists in the database.
    # TODO: refactor below, we have to use the model_name_solr_field value from each doc
    # to constantize appropriate class to call `#where` on
    # At the moment using placeholder Curator::Model, which isn't a real class
    ...
    def self.solr_orphan_ids(batch_size: 100, solr_url: Curator.config.solr_url)
      return enum_for(:solr_index_orphan_ids) unless block_given?

      model_name_solr_field = Curator.config.indexable_settings.model_name_solr_field
      model_solr_id_attr = Curator.config.indexable_settings.solr_id_value_attribute
    ...
    # (or `Curator.config.indexable_settings.model_name_solr_field` if non-default), but don't
    ...
    def self.delete_solr_orphans(batch_size: 100, solr_url: Curator.config.solr_url)
      rsolr = RSolr.connect url: solr_url
      deleted_ids = []
    ...
    def self.delete_all(solr_url: Curator.config.solr_url, commit: :hard)
      rsolr = RSolr.connect url: solr_url
