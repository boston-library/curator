...
RSpec.describe Curator::Indexer do
  ...
  describe 'indexing' do
    ...
    it 'sets the timestamp fields correctly' do
      expect(indexed['system_create_dtsi']).to eq [institution.created_at.to_fs(:iso8601)]
      expect(indexed['system_modified_dtsi']).to eq [institution.updated_at.to_fs(:iso8601)]
    end
  end
end
