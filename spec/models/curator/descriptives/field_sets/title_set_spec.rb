  ...
  describe 'attributes' do
    ...
    describe 'attr_json settings' do
      ...
      it 'expects the attributes to have the following types' do
        ...
        expect(primary_attr_model).to be(Curator::Descriptives::Title)
        expect(other_attr_type).to be_a_kind_of(AttrJson::Type::Array)
        expect(other_attr_model).to be(Curator::Descriptives::Title)
      end

      it 'expects the attributes to have types that match values' do
        expect(subject.primary).to be_a_kind_of(Curator::Descriptives::Title)
        expect(subject.other).to be_a_kind_of(Array).and all(be_an_instance_of(Curator::Descriptives::Title))
      end
    end
  end
end
