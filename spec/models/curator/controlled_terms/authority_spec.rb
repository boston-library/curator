...
RSpec.describe Curator::ControlledTerms::Authority, type: :model do
  ...
  # functionality for fetching canonical name is deprecated
  ...
  describe 'canonical authority' do
    describe '.canonical_json_format' do
      ...
      it 'expects the correct response from authority instance' do
        expect(subject.canonical_json_format).to be_nil
        expect(skos_auth.canonical_json_format).to be('.skos.json')
        expect(jsonld_auth.canonical_json_format).to be('.jsonld')
      end
    end
    ...
    #     authority.name = nil
    #     expect(authority.send(:should_fetch_canonical_name?)).to be_truthy
    #     VCR.use_cassette('controlled_terms/authority_canonicable') do
